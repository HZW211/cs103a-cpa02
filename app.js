...
app.get("/", (req, res, next) => {
  res.locals.days = ['Monday','Tuesday','Wednesday','Thursday','Friday']
  res.render("index");
});
...
app.get('/upsertDB',
  async (req,res,next) => {
    ...
    for (dayevent of events){
      // const {subject,coursenum,section,term}=course;
      const {day, thisevent} = dayevent
      // const num = getNum(coursenum);
      // course.num=num
      // course.suffix = coursenum.slice(num.length)
      dayevent.day = day
      dayevent.event = thisevent
      // await Course.findOneAndUpdate({subject,coursenum,section,term},course,{upsert:true})
      await Events.findOneAndUpdate({day, thisevent},dayevent,{upsert:true})
    }
    // const num = await Course.find({}).count();
    const num = await Events.find({}).count();
...
app.post('/events/show',
  async (req,res,next) => {
    const {day} = req.body;
    const todayevents = await Events.find({day: day})

    console.log(todayevents)
    res.locals.todayevents = todayevents
    res.render('schedule')
  }
)
...
app.post('/events/add',
  async (req,res,next) => {
    const {day, newevent} = req.body;
    const newEv = {day: day, thisevent: newevent}
    await db.collection('events').insertOne(newEv)
    res.redirect('/')
  }
)
...
app.post('/events/delete',
  async (req,res,next) => {
    const {deleteday, deleteevent} = req.body
    console.dir(req.body)
    await db.collection('events').deleteOne({day: deleteday, thisevent: deleteevent})
    res.redirect('/')
  }
)
